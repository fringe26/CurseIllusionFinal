...
namespace CodeBase
{
    public class Resize : MonoBehaviour
    {
        ...
        void ResizeTarget()
        {
            ...
            if (Physics.Raycast(cameraTransform.position, cameraTransform.forward, out var hit, Mathf.Infinity,
                    ignoreTargetMask))
            {
                target.position = hit.point;
                var targetPosition = target.position;
                var currentDistance = Vector3.Distance(cameraTransform.position, targetPosition);
            ...
            {
                target.position = cameraTransform.position + cameraTransform.forward * distance;
            }
        }
    }
}